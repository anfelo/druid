...
druid_update :: proc(state: ^DruidState) {
	...
	if (!rl.CheckCollisionPointRec(state.mouse_position, toolbar_rect)) {
		...
		case .Rectangle:
			...
			if (state.creation.mouse_mode == .Creating && state.creation.resizing_idx != -1) {
				...
				obj.scale.x = (state.mouse_position.x - obj.position.x)
				obj.scale.y = (state.mouse_position.y - obj.position.y)
		...
		case .Eraser:
			...
			if (state.deletion.mouse_pressed) {
				for obj, idx in state.objects {
					...
					if (rl.CheckCollisionPointRec(state.mouse_position, obj_rect) && !ok) {
	...
	druid_set_mouse_cursor(state)

}

druid_draw :: proc(state: ^DruidState) {
...
druid_set_mouse_cursor :: proc(state: ^DruidState) {
	switch state.toolbar.selected {
	case .Selection:
